In Swedish assessment research, comparative judgment (CJ) remains marginal and is often regarded as impractical and too time-consuming. The international literature gives a more specific answer. A single comparison takes considerably less time than an absolute grading of the same texts. Reaching a reliable scale requires many comparisons per text, and the total time then becomes comparable to, not lower than, reliable absolute grading. At that cost, CJ produces a scale that depends less on individual rater severity, and it is already used in both high-stakes and low-stakes assessment of writing.
...
In CJ, a rater reads two responses to the same task and decides which one better demonstrates the intended quality. The rater does not produce two grades and compare them afterward. In an Austrian national EFL test, a subset of judges who did not read the scripts in detail made fast and efficient decisions, and the authors conclude that judges could judge both fast and reliably (Sickinger et al., 2025).
...
The number of comparisons remains the main practical cost at scale (Kinnear et al., 2025). The project's LLM-as-judge addresses this cost. In the Study 1 design, human comparisons are limited to the candidate anchor essays; they are not repeated for every new set of student essays.
...
| New Zealand, NCEA English (Marshall et al., 2020)                           | Official boundary scripts included in the comparisons; their scale positions used to project achievement levels         | Reliability .85; judge time about 7 min per script, similar to traditional marking                                                                     |
...
| Australia, classroom essay assessment (Humphry & Heldsinger, 2019, 2020)    | Teachers first calibrate exemplars through pairwise comparison, then assess new essays against the calibrated exemplars | Stage-1 reliability .96 with 23 teachers after 30 minutes of training                                                                                  |
| Flanders, primary writing (Bouwer et al., 2024)                             | CJ compared with benchmark rating by trained and untrained raters                                                       | Reliability .82 and .79 at 15 comparisons per text                                                                                                     |

The two-stage method of Humphry and Heldsinger (2019, 2020) and the boundary-script design of Marshall et al. (2020) are the closest precedents for Study 1: a scale produced through pairwise comparison becomes the reference against which later essays are placed.
...
The panel workload can be estimated in advance. For illustration, the table assumes 40 candidate anchor essays, a panel of six teachers, random pairing, and 1.75–4.2 minutes per comparison. Panel discussion time is not included.
...
- The evidence on total cost points in different directions: Coertjens et al. (2017) report parity, Steedle and Ferrara (2016) about 17 percent more time for CJ, and Bouwer et al. (2024) warn that CJ can take longer.
